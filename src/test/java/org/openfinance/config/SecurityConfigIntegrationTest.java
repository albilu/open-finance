...
@TestPropertySource(properties = {"jwt.secret=01234567890123456789012345678901","spring.main.allow-bean-definition-overriding=true"})
...
class SecurityConfigIntegrationTest {
    ...
    @Test
    @DisplayName("Should deny access to protected endpoint /health without authentication")
    void shouldDenyHealthWhenUnauthenticated() throws Exception {
        // The app secures /health by default (not the /api path in SecurityConfig),
        // unauthenticated requests are denied. Expect 403 Access Denied.
        mockMvc.perform(get("/health")).andExpect(status().isForbidden());
    }
    ...
    void shouldExposePasswordEncoderAndCors() throws Exception {
        ...
        org.junit.jupiter.api.Assertions.assertNotNull(passwordEncoder, "PasswordEncoder bean should be present");

        // Act - perform CORS preflight
        org.springframework.test.web.servlet.MvcResult result =
                mockMvc
                        .perform(
                                org.springframework.test.web.servlet.request.MockMvcRequestBuilders.options("/health")
