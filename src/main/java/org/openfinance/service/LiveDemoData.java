...
public class LiveDemoData implements CommandLineRunner {
        ...
        public void seedDemoData() {
                ...
                // Only when encryption is globally enabled — otherwise login never derives a
                // key and the
                // seeded data would be unreadable ciphertext (converters pass through when key
                // is null).
                SecretKey encryptionKey = null;
                if (encryptionProperties.isEnabled()) {
                        encryptionKey = keyManagementService.deriveKey(DEMO_MASTER_PASSWORD.toCharArray(), salt);
                ...
                try {
                        ...
                        // Skipped when encryption is disabled, matching normal (non-demo) behavior
                        // where no
                        // key means no search-token indexing.
         ...
         */
        private void generateSearchTokens(
                        Long userId, SecretKey encryptionKey, List<Account> accounts) {
                try {
                        ...
                        log.info(
                                        "Generated search tokens for {} transactions, {} accounts, {} assets, {} liabilities, {} properties",
                                        transactions.size(),
                                        accounts.size(),
                                        assets.size(),
                                        liabilities.size(),
