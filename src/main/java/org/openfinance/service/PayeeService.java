...
public class PayeeService {
    ...
    public List<PayeeResponse> getActivePayees(Long userId) {
        ...
        return payeeRepository.findAllActiveByUserOrderBySystemFirst(userId).stream()
                .sorted(
                        Comparator.comparing((Payee p) -> !Boolean.TRUE.equals(p.getIsSystem()))
                                .thenComparing(
                                        p -> p.getName() != null ? p.getName().toLowerCase() : ""))
                .map(p -> toResponse(p, null, null))
                .toList();
    ...
    /**
     * Find a payee by name (case-insensitive) visible to the given user. Since name is encrypted,
     * SQL equality won't work — fetch all and filter in Java.
     */
    ...
    /**
     * Check if a payee with the given name exists for this user. Since name is encrypted, SQL
     * equality won't work — fetch all and filter in Java.
     */
