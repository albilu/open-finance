...
public class DashboardService {
    ...
    public Map<String, BigDecimal> getCashFlow(Long userId, int period) {
        ...
        // Internal transfer legs (transferId != null) are excluded — moving money
        // between
    ...
    private CashflowSankeyDto buildCashflowSankey(
        ...
        java.util.function.Function<String, Long> resolveCategoryId =
                key -> {
    ...
    public EstimatedInterestSummary getEstimatedInterestSummary(Long userId, String period) {
        ...
        for (Account account : accounts) {
            if (Boolean.TRUE.equals(account.getIsInterestEnabled())) {
                ...
                // Interest is computed in the account's own currency; convert to base for
                // totals.
                earned = convertToBase(earned, account.getCurrency(), baseCurrency);
