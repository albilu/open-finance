...
public class GlobalExceptionHandler {
    ...
    /**
     * Handles client-aborted requests (browser/proxy closed the connection before the response was
     * fully written).
     *
     * <p>This commonly happens on long-running endpoints (e.g. large import confirmation) when the
     * client times out. The server-side work has typically already completed and committed, so this
     * is not an application error. Attempting to write an error response would fail on the already
     * closed socket, so we return {@code null} and log at DEBUG level to avoid noise.
     *
     * @param ex the exception
     * @return {@code null} (no response can be written to the closed connection)
     */
    @ExceptionHandler(org.apache.catalina.connector.ClientAbortException.class)
    public ResponseEntity<Void> handleClientAbort(
            org.apache.catalina.connector.ClientAbortException ex) {
        log.debug(
                "Client aborted the connection before the response was written: {}",
                ex.getMessage());
        return null;
    }
