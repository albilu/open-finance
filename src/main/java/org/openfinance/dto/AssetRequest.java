...
public class AssetRequest {
    ...
    @Digits(integer = 26, fraction = 18, message = "{asset.quantity.digits}")
    ...
    @Digits(integer = 26, fraction = 18, message = "{asset.purchase.price.digits}")
    ...
    @Digits(integer = 26, fraction = 18, message = "{asset.current.price.digits}")
    ...
    @AssertTrue(message = "{asset.planned.value.zero}")
    public boolean isPlannedValueValid() {
        return acquisitionType != org.openfinance.entity.AcquisitionType.PLANNED
                || currentPrice == null
                || currentPrice.signum() == 0;
    }

    @AssertTrue(message = "{asset.purchase.date.future}")
    public boolean isAcquisitionDateValid() {
        return purchaseDate == null
                || acquisitionType == org.openfinance.entity.AcquisitionType.PLANNED
                || !purchaseDate.isAfter(LocalDate.now());
    }
}
