...
public class LiabilityRequest {
    ...
    @Digits(integer = 26, fraction = 18, message = "{liability.principal.digits}")
    ...
    /** Distinguishes a paid historical loan from an approved, undrawn commitment. */
    private Boolean previouslyFunded;

    @AssertTrue(message = "{liability.principal.term}")
    public boolean isTermPrincipalValid() {
        return type == LiabilityType.CREDIT_CARD || principal == null || principal.signum() > 0;
    }

    @AssertTrue(message = "{liability.dates.chronology}")
    ...
    @DecimalMin(value = "0.00", message = "{liability.balance.min}")
    @Digits(integer = 26, fraction = 18, message = "{liability.balance.digits}")
    ...
    @com.fasterxml.jackson.annotation.JsonIgnore private boolean realEstateIdPresent;

    public void setRealEstateId(Long value) {
        this.realEstateId = value;
        this.realEstateIdPresent = true;
    }
}
